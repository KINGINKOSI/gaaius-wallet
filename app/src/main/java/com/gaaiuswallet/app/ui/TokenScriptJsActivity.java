        ...
        OnEthCallListener, OnWalletAddEthereumChainObjectListener, OnWalletActionListener
{
    ...
    private void openTokenscriptWebview(Wallet wallet)
    {
        try
        {
            ...
            tokenScriptView.loadUrl(VIEWER_URL + "/?viewType=gaaiuswallet&chain=" + chainId + "&contract=" + token.tokenInfo.address + "&tokenId=" + tokenId);
        }
        catch (Exception e)
        {
            Timber.e(e);
    ...
    private void showErrorDialogIncompatibleNetwork(long callbackId, long requestingChainId, long activeChainId)
    {
        if (getLifecycle().getCurrentState().isAtLeast(Lifecycle.State.STARTED))
        {
            ...
            String message = com.gaaiuswallet.app.repository.EthereumNetworkBase.isChainSupported(requestingChainId) ?
                    getString(R.string.error_eip712_incompatible_network,
                            com.gaaiuswallet.app.repository.EthereumNetworkBase.getShortChainName(requestingChainId),
                            com.gaaiuswallet.app.repository.EthereumNetworkBase.getShortChainName(activeChainId)) :
