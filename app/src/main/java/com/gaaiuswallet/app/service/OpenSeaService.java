...
public class OpenSeaService
{
    ...
    private Request buildRequest(long networkId, String api)
    {
        ...
        if (!TextUtils.isEmpty(apiKey)
                && !apiKey.equals("...")
                && com.gaaiuswallet.app.repository.EthereumNetworkBase.hasRealValue(networkId)
        )
        {
    ...
    public void resetOffsetRead(List<Long> networkFilter)
    {
        ...
        {
            if (com.gaaiuswallet.app.repository.EthereumNetworkBase.hasOpenseaAPI(networkId))
            {
    ...
    {
        if (!com.gaaiuswallet.app.repository.EthereumNetworkBase.hasOpenseaAPI(token.tokenInfo.chainId))
        {
            return Single.fromCallable(() -> "");
        }
        else
